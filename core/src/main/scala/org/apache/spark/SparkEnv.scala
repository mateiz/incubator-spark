...
object SparkEnv extends Logging {
  ...
  private[spark] class Settings(internalConfig: Config) {
    ...
    final val ioMinThreads = configure("spark.core.connection.io.threads.min", 4)
    final val ioMaxThreads = configure("spark.core.connection.io.threads.max", 32)
    final val ioKeepalive = configure("spark.core.connection.io.threads.keepalive", 60)
