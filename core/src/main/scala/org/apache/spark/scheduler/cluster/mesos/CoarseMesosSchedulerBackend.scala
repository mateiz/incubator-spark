...
private[spark] class CoarseMesosSchedulerBackend(
  ...
  val maxCores = sc.settings.maxCores
  ...
  val extraCoresPerSlave = sc.settings.mesosExtraCoresPerSlave
  ...
  override def resourceOffers(d: SchedulerDriver, offers: JList[Offer]) {
    synchronized {
      ...
      for (offer <- offers) {
        ...
        if (totalCoresAcquired < maxCores && mem >= sc.settings.executorMem && cpus >= 1 &&
          ...
          val task = MesosTaskInfo.newBuilder()
            ...
            .addResources(createResource("mem", sc.settings.executorMem))
            .build()
